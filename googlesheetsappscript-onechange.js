var publicWorkflowUrl = PropertiesService.getScriptProperties().getProperty('publicWorkflowUrl');

function sendSelectedSheetRowChangeToWebhook(e) {
  const webhookUrl = publicWorkflowUrl; // <<< IMPORTANT: GO to Project Settings > Script Propeties; create variable called `publicWorkflowUrl` and set value as public workflow URL of tray webhook triggered wf
  const sheetsToMonitor = ["something else", "Sheet2"]; // <<< IMPORTANT: List the names of sheets you want to monitor
  ...
  const options = {
    ...
    // Uncomment and set your CSRF token if your webhook requires it
    // headers: {
